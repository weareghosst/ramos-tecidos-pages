...
export default function AdminOrderDetailsPage({ order }: Props) {
  ...
  // hooks acima, verificação abaixo — sem erro de TypeScript
  if (!order) {
  ...
  async function handleSendTracking() {
    if (!trackingCode.trim()) {
      alert("Informe o código de rastreio antes de enviar.");
      return;
    }
    try {
      ...
      alert(`E-mail com código de rastreio enviado para ${order!.email}!`);
      window.location.reload();
    } catch (error: any) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="card p-6 space-y-3">
          ...
          <p>
            ...
            {order.tracking_code ? (
              <span className="font-mono font-semibold">{order.tracking_code}</span>
            ) : "-"}
          </p>
        </div>
      </div>
      ...
      <div className="card p-6 space-y-4">
        ...
        <p className="text-sm text-slate-500">
          Cole o código gerado pelo Melhor Envio e clique em enviar. O cliente receberá um e-mail automaticamente.
        </p>
        <div className="flex gap-3 items-center">
          ...
          <button
            ...
            disabled={sendingTracking || trackingSent}
            className="btn-primary px-5 py-3 rounded-xl disabled:opacity-50 whitespace-nowrap"
          >
        ...
        <div className="mt-4 space-y-4">
          {order.items.map((item) => (
            <div key={item.id} className="rounded-xl border border-slate-200 p-4">
              ...
              {item.color_name ? (
                <p className="mt-1 text-sm text-slate-600">Cor: <strong>{item.color_name}</strong></p>
              ) : null}

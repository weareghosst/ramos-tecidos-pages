...
export default function Carrinho() {
  ...
  function setMeters(id: string, meters: number) { updateCartMeters(id, meters); loadCart(); }
  ...
  const total = useMemo(() => {
    return items.reduce((acc, item) => acc + item.meters * item.price_per_meter, 0);
  }, [items]);
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center gap-3">
        <Link href="/produtos" className="btn-outline px-4 py-2">Continuar comprando</Link>
        <button type="button" onClick={clearAll} className="btn-outline px-4 py-2">Limpar</button>
      </div>

      {!items.length ? (
        <div className="card p-6">
          <p className="text-slate-600">Seu carrinho está vazio.</p>
          <Link href="/produtos" className="mt-4 inline-flex btn-primary px-4 py-2">Ver produtos</Link>
        </div>
      ) : (
        <div className="grid gap-8 lg:grid-cols-[1fr_360px]">
                ...
                <div key={item.id} className="card p-4">
                  <div className="flex gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="mt-1 text-sm text-slate-500">{formatBRL(item.price_per_meter)} / metro</p>
                      ...
                      <div className="mt-4 flex items-center gap-3">
                        <button type="button" onClick={() => dec(item.id)} className="h-9 w-9 rounded-xl border border-slate-300 hover:bg-slate-50 transition">-</button>
                        <div className="min-w-[96px] text-center text-sm font-medium">{item.meters.toFixed(1).replace(".", ",")}m</div>
                        <button type="button" onClick={() => inc(item.id)} className="h-9 w-9 rounded-xl border border-slate-300 hover:bg-slate-50 transition">+</button>
                      </div>
                    </div>
          ...
          <div className="card h-fit p-6">
            ...
            <p className="mt-4 text-sm text-slate-500">Você paga via Pix e a confirmação é automática.</p>
